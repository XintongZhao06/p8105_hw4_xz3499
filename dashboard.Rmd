...
snow_data <- noaa_data %>%
  filter(!is.na(snwd) & !is.na(avg_temp) & snwd > 0) %>%
  sample_n(min(2000, nrow(.)))
```

### 🌡️ Monthly Temperature Trends
...
```{r}
plot_ly(noaa_data, y = ~tmax, type = "box", 
        name = 'Max Temp', 
        boxpoints = "outliers",
        marker = list(color = '#1E3A8A'),
